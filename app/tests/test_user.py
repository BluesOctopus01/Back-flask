...
def test_hard_delete_user(client):
    fake_user = {
        "username": "Testeurfou13245",
        "first_name": "TestFirst",
        "last_name": "TestLast",
    ...
    token = response_loading.get_json()["token"]
    headers_delete = {"Authorization": f"Bearer {token}"}
    # Vérification is deleted
    response_self_delete = client.delete("/users/delete/1", headers=headers_delete)
    assert response_self_delete.status_code == 200

    # Verification du get pour voir si l'user n'existe plus
    response_get_user = client.get(f"users/profiles/1")
